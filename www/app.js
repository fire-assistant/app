...
const RG_FACILITY_DESCS = {
  ...
  'ac01': [
    rgField('구획 방식', '제연구역과 제연경계 구획 방식을 기재합니다.'),
    rgField('설치장소', '설치 위치를 동명과 층 단위로 확인합니다.'),
    rgField('제연 방식', '자연제연인지 기계제연인지 확인하여 기재합니다.'),
  ],
  'ac02': [
...
(function initDevLetter() {
  ...
  const LETTER_INTRO = `센터에서 예방 업무를 맡을 때도, 화재안전조사 업무를 할 때도 가장 어려운 부분은 법을 알고 적용하는 일이었습니다. 매일 보던 내용도 시간이 지나면 잊어버리고, 법령도 자주 바뀌어 그때마다 다시 찾아야 했습니다.

예방 업무가 궁금할 때 선배님께 여쭤보면, 법을 펼치지 않고도 바로 답해주시거나 법제처에서 해당 조항을 순식간에 찾아 설명해주시는 모습이 정말 멋있었습니다. 하지만 그런 선배님이 항상 곁에 계신 것도 아니었고, 계시더라도 매번 물어보기가 죄송했습니다.
...
1. 막연한 생각을 도구로
처음에는 막연한 생각으로 시작했습니다. 소방관도 AI를 현장에서 제대로 활용할 수 있지 않을까 고민하던 중, 예전부터 원했던 기능인 건물 규모를 입력하면 소방시설을 나열해주는 것을 실현해보기로 했습니다. 하지만 성능 좋은 AI도 복잡한 소방 법령을 정확하게 다루는 데는 한계가 있었습니다. ChatGPT, Gemini, Claude 같은 여러 도구로 시도해봤지만, 연도별로 바뀌는 소방 법령을 원하는 만큼 처리하기는 어려웠습니다. 결국 AI를 활용하되 직접 수작업으로 만들어보기로 했습니다. 대단한 기술이 있어서가 아니라, 현장에서 필요하다고 느낀 것을 끝까지 실현해보고 싶었습니다.

2. 만든 사람 이야기
솔직히 말하면 저는 소방기술사나 소방시설관리사를 취득한 전문가도 아니고, 예방과에서 오랜 경험을 쌓은 베테랑도 아닙니다. 아직 배울 것도 많고, 남들이 한 번에 이해하는 내용을 두세 번씩 다시 봐야 할 정도로 머리가 그렇게 좋지도 않습니다.
그럼에도 이걸 만든 건, 센터에서 예방업무를 하면서 복잡한 법을 현장에 맞게 정리하는 과정이 재밌었고, 그걸 다른 사람도 쉽게 접할 수 있으면 좋겠다는 생각이 계속 남아있었기 때문입니다.
...
다만 모든 분의 필요를 다 만족시키기는 어렵습니다. 완벽한 정확도가 필요한 분도 있고, 간단하게 쉽게 쓰는 게 더 중요한 분도 있습니다. 그 사이에서 줄다리기를 하며 최대한 간단하면서도 정확한 도구가 될 수 있도록 노력했습니다.
...
5. 배포를 앞두고
업무를 마친 뒤의 시간과 비번 날의 시간을 조금씩 모아 만들었습니다. 다 만들고 나서도 공개하기까지는 망설임이 있었습니다. 괜히 쓸데없이 나서는 것처럼 보이지 않을까 하는 걱정도 있었습니다. 그래도 제가 소방에서 일하며 받았던 도움과 가르침을 조금이라도 돌려드릴 수 있다면 좋겠다고 생각해서 용기내서 이렇게 배포하게 되었습니다. 부족한 부분이 분명 있을 수 있습니다. 버그 제보, 기능 제안, 법적 오류에 대한 의견을 주시면 확인하고 가능한 범위에서 계속 고쳐가겠습니다.`;
